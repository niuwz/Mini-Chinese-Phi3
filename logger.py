...
class Logger(object):
    def __init__(self, logger_name: str, level=logging.DEBUG, std_out: bool = True, save2file: bool = False, file_name: str = None) -> None:
        ...
        if std_out == False and save2file == False:
            raise ValueError(
                'args: [std_out, save2file], at less one of them must be True')
        ...
        if save2file:
            ...
            if file_name is not None:
                ...
                log_file = base_dir + '/' + logger_name + '-' + \
                    str(time.strftime('%Y%m%d', time.localtime())) + '.log'
...
if __name__ == "__main__":
    log = Logger('test', std_out=True, save2file=True,
